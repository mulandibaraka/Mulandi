   ...
   try {
      ...
      if ((body && body.length != 1 && (isPrefix = (myPrefix || '')[0])) || body && commands.includes((body.split` ` [0]).toLowerCase())) {
         ...
         if (!commands.includes(command) && matcher.length > 0 && !setting.self) {
            ...
            if (!m.isGroup || (m.isGroup && !groupSet.mute)) return client.reply(m.chat, `The Command you are using is wrong, try the following recommendations :\n\n${matcher.map(v => '➠ *' + (isPrefix ? isPrefix : '') + v.string + '* (' + v.accuracy + '%)').join('\n')}`, m)
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'confess', 'create_bot'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, `❌ ACCESS DENIED!\n\nCurrently the bot has been set to work in groups only and any subsequent commands or messages will be ignored and marked as read! Try again later!`, m, {

               largeThumb: true,

               thumbnail: await Func.fetchBuffer('https://telegra.ph/file/7d85ae3f7620b677062d7.jpg'),
            ...
            if (cmd.restrict && !isOwner && text && new RegExp('\\b' + global.db.setting.toxic.join('\\b|\\b') + '\\b').test(text.toLowerCase())) {
               ...
               client.reply(m.chat, `You violated the *Terms & Conditions* of using bots by using blacklisted keywords, as a penalty for your violation being blocked and banned. To unblock and unbanned you have to contact owner.`, m).then(() => {
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && chats && !isPrem && !users.banned && !['chatAI'].includes(name) && setting.groupmode) return client.sendMessageModify(m.chat, `❌ ACCESS DENIED!\n\nCurrently the bot has been set to work in groups only and any subsequent commands or messages will be ignored and marked as read! Try again later!`, m, {
